...
define([], function(){
	...
	(function (a, e) {
	    var c = "kik-canvas-fix";
	    if (b()) {
	        d()
	    }

	    function b() {
	        var g = e.utils.platform.os,
	            f = false;
	        if (e.enabled && g.android && (g.version >= 4)) {
	            Array.prototype.forEach.call(a.getElementsByTagName("meta"), function (h) {
	                if ((h.name === c) && (h.content === "true")) {
	                    f = true
	                }
	            })
	        }
	    ...
	    function d() {
	        var f = a.createElement("canvas"),
	            h = f.getContext("2d");
	        for (var g = 0; g < 10000; g += 100) {
	            setTimeout(function () {
	                h.fillStyle = "#fff";
	                h.fillRect(0, 0, 100, 100)
	            }, g)
	        }
	    }
	})(document, cards);
	...
	(function (b, a, c) {
	    if (c._.bridge && c.utils.platform.os.android && b.navigator) {
	        b.navigator.geolocation = undefined
	    }
	})(window, document, cards);
	(function (d) {
	    var b = {};
	...
	(function (j, p, o) {
	    function s(B, F) {
	        var A = j.applicationCache,
	            E = false;
	        if (!A || !A.addEventListener || !A.swapCache || !A.update) {
	            F(false);
	            return
	        }
	        ...
	        if ((A.status !== A.IDLE) && (A.status !== A.CHECKING) && (A.status !== A.DOWNLOADING)) {
	            F(false);
	            return
	        }
	        A.addEventListener("noupdate", C, false);
	        A.addEventListener("updateready", z, false);
	        A.addEventListener("error", z, false);
	        A.addEventListener("obsolete", z, false);
	        setTimeout(z, 30 * 1000);
	        if (A.status === A.IDLE) {
	            try {
	                A.update()
	            } catch (D) {
	        ...
	        function C() {
	            ...
	            E = true;
	            if (!B && j.console && j.console.log) {
	                j.console.log("refresh requested but no update to manifest found");
	                j.console.log("** update your manifest to see changes reflected")
	            }
	            setTimeout(function () {
	                F(true)
	            }, 1000)
	    ...
	    if (o.utils.platform.os.ios) {
	        setTimeout(function () {
	            s(true, function (z) {})
	        }, 5000)
	    }
	    j.ZERVER_REFRESH = function () {
	        c();
	        s(true, function () {
	            j.location.reload()
	        })
	    ...
	    if (j.location.hash) {
	        o.linkData = i(j.location.hash.substr(1))
	    }
	    var a;
	    try {
	        a = o._.bridge("Browser")
	    } catch (d) {
	        return
	    }
	    var w = o.events();
	    ...
	    if (a.setCardInfo) {
	        var m = /(^|\s)icon(\s|$)/i,
	            u, g, x, h;
	        Array.prototype.forEach.call(p.getElementsByTagName("link"), function (z) {
	            if (!u) {
	                if ((z.rel === "kik-icon") || (m.test(z.rel) && !u)) {
	                    u = z.href
	                }
	            }
	            if (!g) {
	                if (z.rel === "kik-tray-icon") {
	                    g = z.href
	                }
	            }
	            if (!x) {
	    ...
	    if (a.pageLoaded) {
	        o.utils.windowReady(function () {
	            if (p.body) {
	                var z = p.body.offsetWidth;
	                (function (A) {
	                    return A
	                })(z)
	            }
	    ...
	    j.addEventListener("unload", function () {
	        try {
	    ...
	    if (a.setStatusBarTransparent) {
	        w.statusBarTransparent = function (z) {
	            try {
	                if (a.setStatusBarTransparent({
	                    transparent: !! z
	                })) {
	    ...
	    if (a.getOrientationLock && a.setOrientationLock) {
	        w.getOrientationLock = function () {
	            var z = a.getOrientationLock().position;
	            return (z === "free") ? null : z
	        };
	        w.setOrientationLock = function (z) {
	            switch (z) {
	            case "free":
	            case "portrait":
	            case "landscape":
	            ...
	            default:
	                ...
	                throw TypeError("if defined, position " + z + ' must be one of "free", "portrait", or "landscape"')
	            }
	            try {
	                a.setOrientationLock({
	                    position: z
	                });
	                if (!k && a.setStatusBarVisible && (r !== (z !== "landscape"))) {
	                    a.setStatusBarVisible({
	                        visible: (z !== "landscape")
	                    });
	                    r = (z !== "landscape")
	    ...
	    w.back = function (z) {
	        if (typeof z !== "function") {
	            throw TypeError("back handler " + z + " must be a function")
	        }
	        e.push(z)
	    };
	    w.unbindBack = function (A) {
	        if (typeof A !== "function") {
	            throw TypeError("back handler " + A + " must be a function")
	        }
	    ...
	    w.back(function () {
	        if (j.App && (typeof j.App.back === "function")) {
	            try {
	                if (App.back() !== false) {
	                    return false
	                }
	    ...
	    if (a.refresh && a.refreshPlanned) {
	        w.refresh = function () {
	            var z = j.applicationCache;
	            c();
	            if (!z || (z.status === z.UNCACHED)) {
	                a.refresh({
	                    withCache: false
	                });
	                return
	    ...
	    if (a.openCard && a.openExternal) {
	        w.open = function (B, A, C) {
	            if (typeof B !== "string") {
	                throw TypeError("url " + B + " must be a string")
	            }
	            switch (typeof A) {
	            case "undefined":
	            case "string":
	            ...
	            case "object":
	                ...
	                break;
	            default:
	                throw TypeError("card linkData must be a string or JSON if defined, got " + A)
	            }
	            switch (typeof C) {
	            case "undefined":
	                C = {};
	            ...
	            case "object":
	                break;
	            default:
	                throw TypeError("card data must be an object if defined, got " + C)
	            }
	            var z = B.substr(0, 7) === "card://",
	                D = B.substr(0, 8) === "cards://";
	            if (!z && !D) {
	                a.openExternal({
	                    url: B
	                });
	            ...
	            B = "http" + B.substr(4);
	            if (A) {
	                B = B.split("#")[0] + "#" + encodeURIComponent(A)
	            }
	            a.openCard({
	                url: B,
	    ...
	    function q(A, z) {
	        w.linkData = i(A && A.data);
	        o.linkData = w.linkData;
	        if ((z !== false) && w.linkData) {
	            w.trigger("linkData", w.linkData)
	        }
	    }
	    w._processLinkData = q;
	    if (a.getLastLinkData) {
	        q(cards._.firstBatch["Browser.getLastLinkData"] || a.getLastLinkData(), false)
	    }
	    a.on("linkData", q);
	    var n = true,
	        v = false,
	        t = true;
	    w.background = n;
	    a.on("pause", function (z) {
	        v = true;
	        f()
	    });
	    a.on("unpause", function (z) {
	        v = false;
	        f()
	    });
	    a.on("conceal", function (z) {
	        t = true;
	        f()
	    });
	    a.on("reveal", function (z) {
	        t = false;
	        f()
	    });
	    ...
	    function f() {
	        ...
	        w.background = n;
	        if (z !== n) {
	            w.trigger(n ? "background" : "foreground")
	        }
	    }
	})(window, document, cards);
	(function (g, c, f) {
	        ...
	        var e = function () {
	            ...
	            try {
	                if (h._.firstBatch && h._.firstBatch["Push.getNotificationList"]) {
	                    o = h._.firstBatch["Push.getNotificationList"]
	                } else {
	                    o = a.getNotificationList()
	                }
	            } catch (n) {}
	            if (a.setBadgeVisibility && !(h._.firstBatch && h._.firstBatch["Push.setBadgeVisibility"])) {
	                try {
	                    a.setBadgeVisibility({
	                        visible: false
	                    })
	            ...
	            if (o && o.notifications) {
	                var m = [];
	                for (var k = 0, j = o.notifications.length; k < j; k++) {
	                    if (typeof o.notifications[k] === "object") {
	                        switch (typeof o.notifications[k].data) {
	                        case "object":
	                            m.push(o.notifications[k].data);
	                            break;
	                        case "string":
	                            try {
	                                var p = JSON.parse(o.notifications[k].data);
	                                if (typeof p === "object") {
	                                    m.push(p)
	                                } else {}
	                            } catch (n) {}
	...
	(function (f, g, b) {
	    ...
	    if (!h.purchase || !h.markTransactionStored || !h.getAvailableItems || !h.getTransactionList) {
	        return
	    }
	    var c = b.events(a);
	    c.init = i;
